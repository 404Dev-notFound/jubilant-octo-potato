...
function animate() {
    ...
    const baseCameraY = 40;
    const targetY = baseCameraY - (scrollProgress * 120);
    const targetZ = Math.cos(time * 0.1) * 100 - (scrollProgress * 20);
    const targetX = Math.sin(time * 0.1) * 100;
    ...
    const count = COUNT;
    for (let i = 0; i < COUNT; i++) {
        const nebulaSize = addControl("nebulaSize", "Rose Bloom Radius", 12.0, 48.0, 30.0);
        const petalCurl = addControl("petalCurl", "Petal Curl", 0.2, 2.4, 1.15);
        const rotationRate = addControl("rotationRate", "Nebular Rotation", 0.0, 1.5, 0.32);
        const cloudDepth = addControl("cloudDepth", "Cloud Depth", 0.5, 8.0, 3.4);
        const corePulse = addControl("corePulse", "Core Pulse", 0.0, 2.5, 0.75);
        const turbulence = addControl("turbulence", "Gas Turbulence", 0.0, 2.0, 0.7);
        ...
        // 14% OF PARTICLES: Dense central cluster
        ...
        // 12% OF PARTICLES: Diffuse halo
            ...
            const haloWarp = 1.0 + 0.10 * Math.sin(haloAngle * 5.0 + time * 0.25) + 0.05 * turbulence * Math.sin(haloAngle * 11.0 - time * 0.17);
        ...
        // 4% OF PARTICLES: Background stars
        else {
        ...
        if (i === 0) {
            ...
            annotate("rose_core", new THREE.Vector3(0, 0, 0), "Young Star Cluster");
            annotate("rose_petals", new THREE.Vector3(nebulaSize * 0.58, cloudDepth * 0.4, 0), "Ionized Gas Petals");
            annotate("rose_halo", new THREE.Vector3(-nebulaSize * 1.05, -cloudDepth * 0.3, 0), "Outer Nebular Halo");
        }
